...
function SVM(){
    ...
    const generateSampleDataWithOptions = () => {
        ...
        getSVMSampleData(sampleDataType, sampleCount, sampleClusters, sampleVariance)
        .then(response => {
            if (response && response.X && response.y) {
                ...
                for (let i = 0; i < response.X.length; i++) {
                    pairs.push({
                        x: parseFloat(response.X[i][0]).toFixed(2),
                        y: parseFloat(response.X[i][1]).toFixed(2),
                        class: parseInt(response.y[i])
                    });
                }
